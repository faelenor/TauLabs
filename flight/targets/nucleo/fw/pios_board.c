...
void PIOS_Board_Init(void) {
	...
	if (boot_count < 3) {
		...
		HwNucleoSetDefaults(HwNucleoHandle(), 0);
	...
	HwNucleoUSB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWNUCLEO_USB_VCPPORT_DISABLED;
	}
	...
	HwNucleoUSB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWNUCLEO_USB_HIDPORT_DISABLED;
	}
	...
	HwNucleoDSMxModeOptions hw_DSMxMode;
	HwNucleoDSMxModeGet(&hw_DSMxMode);

	/* UART1 Port */
	uint8_t hw_uart1;
	HwNucleoUart1Get(&hw_uart1);
	...
	uint8_t hw_rcvrport;
	HwNucleoRcvrPortGet(&hw_rcvrport);

	switch (hw_rcvrport) {
	case HWNUCLEO_RCVRPORT_DISABLED:
		break;
	
	case HWNUCLEO_RCVRPORT_PWM:
	...
	case HWNUCLEO_RCVRPORT_PPM:
	case HWNUCLEO_RCVRPORT_PPMADC:
	case HWNUCLEO_RCVRPORT_PPMOUTPUTS:
	case HWNUCLEO_RCVRPORT_PPMOUTPUTSADC:
	...
	case HWNUCLEO_RCVRPORT_PPMPWMADC:
		PIOS_HAL_ConfigurePort(HWSHARED_PORTTYPES_PPM,  // port type protocol
				NULL,                                   // usart_port_cfg
				NULL,                                   // frsky usart_port_cfg
				NULL,                                   // com_driver
	...
	switch (hw_rcvrport) {
		...
		case HWNUCLEO_RCVRPORT_PPMPWMADC:
			...
			break;
		case HWNUCLEO_RCVRPORT_PPMOUTPUTS:
		case HWNUCLEO_RCVRPORT_OUTPUTS:
#ifdef PIOS_INCLUDE_SERVO
			PIOS_Servo_Init(&pios_servo_with_rcvr_cfg);
#endif
			break;
		case HWNUCLEO_RCVRPORT_PPMOUTPUTSADC:
		case HWNUCLEO_RCVRPORT_OUTPUTSADC:
#ifdef PIOS_INCLUDE_SERVO
...
/* init sensor queue registration */
	...
	HwNucleoMPU6050DLPFGet(&hw_mpu6050_dlpf);
	enum pios_mpu60x0_filter mpu6050_dlpf = \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_256) ? PIOS_MPU60X0_LOWPASS_256_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_188) ? PIOS_MPU60X0_LOWPASS_188_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_98) ? PIOS_MPU60X0_LOWPASS_98_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_42) ? PIOS_MPU60X0_LOWPASS_42_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_20) ? PIOS_MPU60X0_LOWPASS_20_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_10) ? PIOS_MPU60X0_LOWPASS_10_HZ : \
	    (hw_mpu6050_dlpf == HWNUCLEO_MPU6050DLPF_5) ? PIOS_MPU60X0_LOWPASS_5_HZ : \
	    pios_mpu6050_cfg.default_filter;
	PIOS_MPU6050_SetLPF(mpu6050_dlpf);

	uint8_t hw_mpu6050_samplerate;
	HwNucleoMPU6050RateGet(&hw_mpu6050_samplerate);
	uint16_t mpu6050_samplerate = \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_200) ? 200 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_333) ? 333 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_500) ? 500 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_666) ? 666 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_1000) ? 1000 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_2000) ? 2000 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_4000) ? 4000 : \
	    (hw_mpu6050_samplerate == HWNUCLEO_MPU6050RATE_8000) ? 8000 : \
...
#if defined(PIOS_INCLUDE_HMC5883)
	{
		...
		HwNucleoMagnetometerGet(&Magnetometer);

		if (Magnetometer == HWNUCLEO_MAGNETOMETER_INTERNAL) {
		...
		if (Magnetometer == HWNUCLEO_MAGNETOMETER_EXTERNALI2CUART1) {
			// init sensor
		...
		if (Magnetometer == HWNUCLEO_MAGNETOMETER_EXTERNALI2CUART3) {
			// init sensor
		...
		if (Magnetometer == HWNUCLEO_MAGNETOMETER_EXTERNALI2CUART1 ||
			Magnetometer == HWNUCLEO_MAGNETOMETER_EXTERNALI2CUART3)
		{
			// setup sensor orientation
			uint8_t ExtMagOrientation;
			HwNucleoExtMagOrientationGet(&ExtMagOrientation);

			enum pios_hmc5883_orientation hmc5883_orientation = \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_TOP0DEGCW) ? PIOS_HMC5883_TOP_0DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_TOP90DEGCW) ? PIOS_HMC5883_TOP_90DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_TOP180DEGCW) ? PIOS_HMC5883_TOP_180DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_TOP270DEGCW) ? PIOS_HMC5883_TOP_270DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_BOTTOM0DEGCW) ? PIOS_HMC5883_BOTTOM_0DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_BOTTOM90DEGCW) ? PIOS_HMC5883_BOTTOM_90DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_BOTTOM180DEGCW) ? PIOS_HMC5883_BOTTOM_180DEG : \
				(ExtMagOrientation == HWNUCLEO_EXTMAGORIENTATION_BOTTOM270DEGCW) ? PIOS_HMC5883_BOTTOM_270DEG : \
	...
	if (hw_rcvrport == HWNUCLEO_RCVRPORT_PWMADC ||
			hw_rcvrport == HWNUCLEO_RCVRPORT_PPMADC ||
			hw_rcvrport == HWNUCLEO_RCVRPORT_PPMPWMADC ||
			hw_rcvrport == HWNUCLEO_RCVRPORT_OUTPUTSADC ||
			hw_rcvrport == HWNUCLEO_RCVRPORT_PPMOUTPUTSADC) {
